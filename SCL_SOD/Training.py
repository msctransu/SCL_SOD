...
import os
from Models.ContrastImageNet import ConrastDepthImageNet
from Models.MTLoss import MtlClass
...
def main(local_rank, num_gpus, args):
    ...
    loss_params = loss_net.parameters()

    optimizer = optim.Adam([{'params': base_params, 'lr': args.lr * 0.1},
                            {'params': other_params, 'lr': args.lr},
                            {'params': loss_params, 'lr':args.loss_lr}])
    ...
    criterion_con = nn.CosineSimilarity(dim=1).cuda()
    whole_iter_num = 0
    ...
    for epoch in range(args.epochs):
        ...
        for i, data_batch in enumerate(train_loader):
            ...
            contour_224, contour_14, contour_28, contour_56, contour_112, arg_images = data_batch
            ...
            image1 = Variable(arg_images[0].cuda())
            image2 = Variable(arg_images[1].cuda())

            print("start evaluate the flops and params: \n")
            start_time = time.time()
            flops, params = profile(net, (image1, image2, images,))
            end_time = time.time()
            ...
            outputs, p1, p2, z1, z2 = net(image1, image2, images)
            outputs_saliency, outputs_contour = outputs

            con_loss = -(criterion_con(p1, z2).mean() + criterion_con(p2, z1).mean()) * 0.5 + 1
            ...
            llists = [loss1, loss3, loss4, loss5, c_loss1, c_loss3, c_loss4, c_loss5, con_loss]
            ...
            # total_loss = img_total_loss + contour_total_loss + con_loss

            total_loss = loss_net(llists)
            ...
            if (local_rank == 0) and (whole_iter_num == args.train_steps):
                torch.save(net.state_dict(), args.save_model_dir + 'SCL_SOD.pth')
